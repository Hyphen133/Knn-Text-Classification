...
public class PlacesTagsLoaderV2 {

    public static ArrayList<String> loadPlacesTagsFromReutersDirectory(){

        ArrayList<String> reuterTags = new ArrayList<>();
        ...
        for (int i = 0; i < Config.REUTERS_SIZE; i++) {
            ...
            reuterTags.addAll(loadPlacesTagsFromFile("reuters/reut2-0" + index + ".sgm"));
        }
    ...
    private static ArrayList<String> loadPlacesTagsFromFile(String filepath) {
        ...
        URL res = ReutersLoaderV2.class.getClassLoader().getResource(filepath);
        File file = null;
        try {
    ...
    public static Map<String,Integer> getAllPlacesMap(){
        ...
        URL res = ReutersLoaderV2.class.getClassLoader().getResource("reuters/all-places-strings.lc.txt");
        File file = null;
        try {
            file = Paths.get(res.toURI()).toFile();
        } catch (URISyntaxException e) {
